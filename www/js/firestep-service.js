...
function($http, service, interpolate, transmit) {
  var firestep = {
    ...
    on_focus: function(tag,key,clearArmed) { 
      ...
      if (clearArmed) {firestep.armed = null;} 
    },
    is_focus: function(tag,key) { return firestep.focus === tag + key; },
    ...
    resource_GET: function(resource) {
      ...
      transmit.start();
      $.ajax({
        url: firestep.resource_url(resource), data: { r: Math.floor(Math.random()*1000000) },
