...
var firepick; (function (firepick) {
  ...
  var DeltaModel = (function () {
    ...
    DeltaModel.prototype.getPrecision = function() { return this.precision; };
    ...
    DeltaModel.prototype.getAngles = function() {
      return {theta1:this.theta1, theta2:this.theta2, theta3:this.theta3};
    };
	...
	return {x:this.round(value.x),y:this.round(value.y),z:this.round(value.z)};
      }
...
(function (firepick) {
  firepick.DeltaModelTest = function () {
    ...
    try {
      ...
      assert.equal(-1.234,fpd.round(-1.2341));
      assert.deepEqual({x:1.234,y:2.345,z:3.456},fpd.round({x:1.2335,y:2.3454,z:3.4559}));
      assert.deepEqual({x:0,y:0,z:-96.859}, fpd.round(fpd.getXYZ()));
      assert.deepEqual({theta1:0,theta2:0,theta3:0}, fpd.getAngles());
      assert.equal(3, fpd.getPrecision());
      assert.equal(fpd, fpd.setPrecision(5));
      assert.equal(5, fpd.getPrecision());
      assert.deepEqual(
        {e:[115,115,115], f:[457.3,457.3,457.3], re:[232,232,232], rf:[112,112,112]},
	fpd.getDimensions());

      // Forward kinematics
      assert.deepEqual({x:0,y:0,z:-96.85902},
        fpd.round(fpd.calcXYZ({theta1:0,theta2:0,theta3:0})));
      assert.deepEqual({x:0,y:0,z:-96.85902},
        fpd.round(fpd.calcXYZ({theta1:0,theta2:0,theta3:0})));
      fpd.setPrecision(6);
      fpd.setAngles({theta1:0,theta2:0,theta3:0});
      assert.deepEqual({x:0,y:0,z:-96.859015}, fpd.round(fpd.calcXYZ(fpd.getAngles())));
      assert.ok(fpd.isValid());
